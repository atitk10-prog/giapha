...
- âœ… Pass a **scope** (ref or element) so selectors like `.box` are scoped to that root.
- âœ… Cleanup (reverting animations and ScrollTriggers) runs automatically on unmount.
- âœ… Use **contextSafe** from the hook's return value to wrap callbacks (e.g. onComplete) so they no-op after unmount and avoid React warnings.
...
- âœ… Pass a **scope** (ref or element) as the second argument so selectors are scoped to that node.
- âœ… **Always** return a cleanup that calls **ctx.revert()**.
...
- âœ… Prefer **useGSAP()** from `@gsap/react` rather than `useEffect()`/`useLayoutEffect()`; use **gsap.context()** + **ctx.revert()** in `useEffect` when `useGSAP` is not an option.
- âœ… Use refs for targets and pass a **scope** so selectors are limited to the component.
- âœ… Run GSAP only on the client (useGSAP or useEffect); do not call gsap or ScrollTrigger during SSR.

## Do Not

- âŒ Target by **selector without a scope**; always pass **scope** (ref or element) in useGSAP or gsap.context() so selectors like `.box` are limited to that root and do not match elements outside the component.
- âŒ Animate using selector strings that can match elements outside the current component unless a `scope` is defined in useGSAP or gsap.context() so only elements inside the component are affected.
- âŒ Skip cleanup; always revert context or kill tweens/ScrollTriggers in the effect return to avoid leaks and updates on unmounted nodes.
- âŒ Run GSAP or ScrollTrigger during SSR; keep all usage inside client-only lifecycle (e.g. useGSAP).
