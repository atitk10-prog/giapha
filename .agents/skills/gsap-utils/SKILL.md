...
Maps a value from one range to another. Use when converting scroll position, progress (0â€“1), or input range to an animation range. Omit **value** to get a function: `mapRange(inMin, inMax, outMin, outMax)(value)`.
...
Returns a value normalized to 0â€“1 for the given range. Inverse of mapping when the target range is 0â€“1. Omit **value** to get a function: `normalize(min, max)(value)`.
...
Interpolates between two values at a given progress (0â€“1). Handles numbers, colors, and objects with matching keys. Omit **progress** to get a function: `interpolate(start, end)(progress)`.
...
gsap.utils.random(0, 500, 5);        // 0â€“500, snapped to nearest 5
...
Converts a color string into an array: **[red, green, blue]** (0â€“255), or **[red, green, blue, alpha]** (4 elements for RGBA when alpha is present or required). Pass **true** as the second argument (**returnHSL**) to get **[hue, saturation, lightness]** or **[hue, saturation, lightness, alpha]** (HSL/HSLA) instead. Works with `"rgb()"`, `"rgba()"`, `"hsl()"`, `"hsla()"`, hex, and named colors (e.g. `"red"`). Use when animating color components or building gradients. See [splitColor()](https://gsap.com/docs/v3/GSAP/UtilityMethods/splitColor/).
...
Wraps a value into the range minâ€“max (inclusive min, exclusive max). Use for infinite scroll or cyclic values. Omit **value** to get a function: `wrap(min, max)(value)`.
...
- âœ… Omit the value argument to get a reusable function when the same range/config is used many times (e.g. scroll handler, tween callback): `let mapFn = gsap.utils.mapRange(0, 1, 0, 360); mapFn(progress)`.
- âœ… Use **snap** for grid-aligned or step-based values; use **toArray** when GSAP or your code needs a real array from a selector or NodeList.
- âœ… Use **gsap.utils.selector(scope)** in components so selectors are scoped to a container or ref.

## Do Not

- âŒ Assume **mapRange** / **normalize** handle units; they work on numbers. Use **getUnit** / **unitize** when units matter.
- âŒ Override or rely on undocumented behavior; stick to the documented API.
