...
- âœ… Install everything from the public `gsap` npm package: `npm install gsap`. All plugins are included â€” import them as `gsap/SplitText`, `gsap/MorphSVGPlugin`, etc.
- âŒ Do **not** generate an `.npmrc` with a GreenSock auth token, suggest the private `npm.greensock.com` registry, or tell users to sign up for Club GSAP to access a plugin. Those instructions are outdated.
...
- âœ… Register before using the plugin in any tween or API call.
- âœ… In React, register at top level or once in the app (e.g. before first useGSAP); do not register inside a component that re-renders. useGSAP is a plugin that needs to be registered before use.

## Scroll

### ScrollToPlugin

Animates scroll position (window or a scrollable element). Use for â€œscroll to elementâ€ or â€œscroll to positionâ€ without ScrollTrigger.
...
| `edgeResistance` | 0â€“1; resistance when dragging past bounds |
...
| **mask** | `"lines"`, `"words"`, or `"chars"`. Wraps each unit in an extra element with `overflow: clip` for mask/reveal effects. Only one type; access wrappers on the instanceâ€™s **masks** array (or use class `-mask` if a class is set). |
...
| **deepSlice** | When `true` (default), nested elements (e.g. `<strong>`) that span multiple lines are subdivided so lines donâ€™t stretch vertically. Only applies when splitting lines. |
...
Reveals or hides the stroke of SVG elements by animating `stroke-dashoffset` / `stroke-dasharray`. Works on `<path>`, `<line>`, `<polyline>`, `<polygon>`, `<rect>`, `<ellipse>`. Use when â€œdrawingâ€ or â€œerasingâ€ strokes.

**drawSVG value:** Describes the **visible segment** of the stroke along the path (start and end positions), not â€œanimate from A to B over time.â€ Format: `"start end"` in percent or length. Examples: `"0% 100%"` = full stroke; `"20% 80%"` = stroke only between 20% and 80% (gaps at both ends). The tween animates from the elementâ€™s **current** segment to the **target** segment â€” e.g. `gsap.to("#path", { drawSVG: "0% 100%" })` goes from whatever it is now to full stroke. Single value (e.g. `0`, `"100%"`) means start is 0: `"100%"` is equivalent to `"0% 100%"`. 
...
// or explicit segment: from 0â€“0 to 0â€“100%
...
| **map** | How segments are matched: `"size"` (default), `"position"`, or `"complexity"`. Use when start/end segments donâ€™t line up; if none work, split into multiple paths and morph each. |
| **shapeIndex** | Offsets which point in the start path maps to the first point in the end path (avoids shape â€œcrossing overâ€ or inverting). Number for single-segment paths; **array** for multi-segment (e.g. `[5, 1, -8]`). Negative reverses that segment. Use **shapeIndex: "log"** once to log the auto-calculated value, then paste the number/array into the tween. **findShapeIndex(start, end)** (separate utility) provides an interactive UI to find a good value. Only applies to closed paths. |
...
| `alignOrigin` | `[x, y]` origin (0â€“1); default `[0.5, 0.5]` |
| `autoRotate` | Rotate element to follow path tangent |
| `curviness` | 0â€“2; path smoothing |
...
Wiggle/shake easing. Use when a value should â€œwiggleâ€ (multiple oscillations).
...
- âœ… Register every plugin used with **gsap.registerPlugin()** before first use.
- âœ… Use **Flip.getState()** â†’ DOM change â†’ **Flip.from()** for layout transitions; use **Draggable** + **InertiaPlugin** for drag with momentum.
- âœ… Revert plugin instances (e.g. `SplitTextInstance.revert()`) when components unmount or elements are removed.

## Do Not

- âŒ Use a plugin in a tween or API without registering it first (**gsap.registerPlugin()**).
- âŒ Ship GSDevTools or development-only plugins to production.
